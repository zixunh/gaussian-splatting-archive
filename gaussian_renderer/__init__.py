...
def render(
    ...
    fetch_prev_next_exp=False,
    render_360_scene=False,
):
    """
    Render the scene.
    ...
    if not render_360_scene:
        tanfovx = math.tan(viewpoint_camera.FoVx * 0.5)
        tanfovy = math.tan(viewpoint_camera.FoVy * 0.5)
    else:  # Actually omni_tanfov with xi = 1
        tanfovx = math.tan(viewpoint_camera.FoVx * 0.5 * 0.5)
        tanfovy = math.tan(viewpoint_camera.FoVy * 0.5 * 0.5)

    scaling_modifier = scaling_modifier
    raster_settings = GaussianRasterizationSettings(
        image_height=(
            int(viewpoint_camera.omni_tan_phi.shape[0])
            if viewpoint_camera.render_camera_model == "BEAP"
            else int(viewpoint_camera.raymap.shape[0])
        ),  # for ray-splatting beap / kb
        image_width=(
            int(viewpoint_camera.omni_tan_theta.shape[0])
            if viewpoint_camera.render_camera_model == "BEAP"
            else int(viewpoint_camera.raymap.shape[1])
        ),  # for ray-splatting
        tanfovx=tanfovx,
        tanfovy=tanfovy,
        bg=bg_color,
        ...
        raymap=viewpoint_camera.raymap.cuda(),
    ...
    if pipe.compute_cov3D_python:
        print(
            "We use the inverse-sigma S^(-1)Rt for rasterizer forwards and backwards; precomputing cov3D is disabled."
        )
