...
def colmap_main(args):
    ...
    for i in tqdm(range(0, width), desc="calculate_reverse_maps"):
        for j in range(0, height):
            ...
            theta_extend = int(np.floor(FoVx / args.step))
            focal_theta = theta_extend / np.tan(FoVx)
    
            phi_extend = int(np.floor(FoVy / args.step))
            focal_phi = phi_extend / np.tan(FoVy)
            
            x2 = focal_theta * tan_theta + theta_extend
            y2 = focal_phi * tan_phi + phi_extend
            # print(num_phi, num_theta, tan_theta_interval, tan_phi_interval)
            reverse_mapx[i, j] = x2
            reverse_mapy[i, j] = y2
