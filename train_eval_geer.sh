...
SCENE_ID="steakhouse_patio" #livingroom #steakhouse_patio # garden
...
FOVMOD_EVAL=1.0 #2.0 #0.85 #0.85 #1.0 #2.0 #1.0 #2.0
...
# # eval
# python prepare_fov.py --path $DATASET_DIR --dst $FOVMAP_DIR_EVAL --step $STEP_EVAL --fov_mod $FOVMOD_EVAL --mask_dst $TEST_MASK_FN --add_ego_mask
...
python metrics.py \
    -m $OUTPUT_DIR \
    --iters $ITERS_NUM \
    --custom_gt $OUTPUT_DIR/test/ours_30000/gt_denoised \
    --custom_mask $DATASET_DIR/$FOVMAP_DIR_EVAL$TEST_MASK_FN \
    # --block_mask \
    # --custom_gt /home/Fisheye-GS/output/aria/$SCENE_ID/test/ours_$ITERS_NUM/gt_denoised_masked \


# python prepare_fov.py --path /home/Fisheye-GS/output_remapped_gt/aria/$SCENE_ID/test/ours_30000/ --cam_path $DATASET_DIR --src renders --dst renders_remap --step $STEP_EVAL --fov_mod $FOVMOD_EVAL --mask_dst $TEST_MASK_FN --add_ego_mask
