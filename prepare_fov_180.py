...
def fov2tan_EQUREC(fovx, fovy, interval):
    ...
    theta_map, phi_map = np.meshgrid(theta_arr, phi_arr, indexing = 'xy')

    tan_theta_map = np.tan(theta_map)

    tan_phi_map = -np.tan(phi_map) / np.sqrt(tan_theta_map * tan_theta_map + 1)

    theta_condition = np.logical_or(theta_map > np.pi / 2, theta_map < -np.pi / 2)
    phi_condition = np.logical_or(phi_map > np.pi / 2, phi_map < -np.pi / 2)

    tan_phi_map *= np.where(np.logical_or(theta_condition, phi_condition), -1, 1)
    phi_map = np.arctan(tan_phi_map) 
...
def colmap_main(args):
    ...
    theta_arr, phi_arr = fov2tan_EQUREC(FoVx, FoVy, args.step)
