...
def fov2tan_pinhole(fovx, fovy, interval):
    ...
    # tantheta_arr = np.arange(taninterval / 2, tanfovx, taninterval)
    # tantheta_arr = np.sort(np.concatenate((-theta_arr, theta_arr)))
    # tanphi_arr = np.arange(taninterval / 2, tanfovy, taninterval)
    # tanphi_arr = np.sort(np.concatenate((-phi_arr, phi_arr)))
